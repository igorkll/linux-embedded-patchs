...
* disable_vt_swithing_from_ioctl.patch - block to switch VT from userspace via ioctl. It's perfect if there's no normal way to disable it in your wayland compositor. with this patch, userspace won'T SWITCH VT if it wants TO. However, if you haven't installed the "disable_vt_swithing_from_keyboard.patch" patch, VT can still be switched from the keyboard.
